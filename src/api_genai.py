...
당신은 이 데이터를 문장 단위로 분석하여 각 거래에 대해 '거래_유형', '주요_카테고리', '세부_카테고리', '판단_사유'를 추론해야 합니다.
...
3. 해당 거래에 가장 적합한 '주요 카테고리'와 '세부 카테고리'를 자율적으로 부여하고
4. 그 사유를 1문장으로 명확히 작성해야 합니다.

* 결과는 반드시 JSON 배열 형식으로만 응답해야 합니다.
"""
# * '주요 카테고리'는 대분류 (예: '생활비', '투자', '소득', '주거'), 
# * '세부 카테고리'는 소분류 (예: '외식', '주식매수', '월급', '관리비')로 구분합니다.
